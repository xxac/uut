...
BOOL Cmd_QueryStoreStatus(char * pbCmd,DWORD CmdLength)
{
    ...
    memset(g_DiskName,0,sizeof(g_DiskName));
    temp = strstr(pbCmd,",");
    tempLength = temp - pbCmd;
...
BOOL Cmd_fwtype(char * pbCmd)
{
    ...
    else if(strncmp(pbCmd+strlen("fwtype:"),"TCB_NB",strlen("TCB_NB")) == 0){//TCB.BIN
        RETAILMSG(1, (_T("UTP command:Image type: TCB NB.\r\n")));
        g_NANDWrtImgInfo.dwImgType = IMAGE_TCB;
        g_fwType = fwType_NK_NB;
    }
	else if(strncmp(pbCmd+strlen("fwtype:"),"VCB_NB",strlen("VCB_NB")) == 0){//VCB.BIN
		RETAILMSG(1, (_T("UTP command:Image type: VCB NB.\r\n")));
		g_NANDWrtImgInfo.dwImgType = IMAGE_VCB;
		g_fwType = fwType_NK_NB;
	}
	else if(strncmp(pbCmd+strlen("fwtype:"),"RSNK_NB",strlen("RSNK_NB")) == 0){//Rescue_nk.nb0
		RETAILMSG(1, (_T("UTP command:Image type: RSNK NB.\r\n")));
		g_NANDWrtImgInfo.dwImgType = IMAGE_RSNK;
		g_fwType = fwType_NK_NB;
	}
...
BOOL Cmd_send(PUTP_MSG pUTPMsg)
{
    ...
    if(g_FileType == Firmware || g_FileType == RawData || g_FileType == Tcb || g_FileType == Vcb)
    {
...
)
{
	...
	else if (strncmp(pbCmd,"wft",3) == 0) { // TCB
		RETAILMSG(1, (_T("UTP command:Write TCB.\r\n"))); 
		g_UtpCmdState = UCE_BEGIN_RECV_FILE;
		g_FileType = Tcb;
	}
	else if (strncmp(pbCmd,"wfv",3) == 0) { // VCB
		RETAILMSG(1, (_T("UTP command:Write VCB.\r\n"))); 
		g_UtpCmdState = UCE_BEGIN_RECV_FILE;
		g_FileType = Vcb;
	}
    else{
