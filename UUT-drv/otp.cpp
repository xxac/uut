...
//
//-----------------------------------------------------------------------------
BOOL SingleOTPRead(POtpProgram pOtp)
{
...
//
//-----------------------------------------------------------------------------
BOOL SingleOTPProgram(POtpProgram pOtp)
{
...
BOOL OTPProgram(char *pBuff)
{
    ...
    Otp.OtpAddr = Char2Int32(pBuff+14);
    Otp.OtpData = Char2Int32(pBuff+25);
               
    //RETAILMSG(1, (_T("To program: Otp address = 0x%x, Otp data = 0x%x.\r\n"),Otp.OtpAddr, Otp.OtpData));

    InitOTP();
    rc = SingleOTPProgram(&Otp);
    DeInitOTP();   
...
BOOL OTPRead(char *pBuff)
{
    BOOL rc;
    
    POtpProgram pOtp = (POtpProgram)pBuff; 

    InitOTP();
    rc = SingleOTPRead(pOtp);
    DeInitOTP();   
